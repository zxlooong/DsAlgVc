...
BTree Insert(BTree T,KeyType k){
 ...
 int i;
 if(BTSearch(T,k,&p)) return T;//在树中搜索k,若找到直接返回,不进行插入
 while(p!=NULL){
   p->key[0]=k;            //设置哨兵
  ...
  if(p==NULL) //需要产生新的根结点
  {p=(BTree)malloc(sizeof(BTreeNode));//申请新结点
...
BTree split(BTree p)
{int i,mid,j;
 BTree New=(BTree)malloc(sizeof(BTreeNode));
 mid=(m+1)/2;
 New->ptr[0]=p->ptr[mid];
 j=1;
 for(i=mid+1;i<=m;i++)
...
{BTree p,s;
 ...
 for(j=i;j<p->keynum;j++)//从*p删除第i个关键字
   p->key[j]=p->key[j+1];
 p->keynum--;
