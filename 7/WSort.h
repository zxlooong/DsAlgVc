...
{public:
  //构造函数
  //向物理文件名为fn指针所指文件中输出n个记录
  LoadFile(char* fn,int n);
  //采用选择排序法对文件A的记录进行排序
...
//类的实现
//向物理文件名为fn指针所指文件中输出n个记录
LoadFile::LoadFile(char* fn,int n)
{fstream f(fn,ios::out|ios::dec|ios::trunc);
 //用所给的物理文件名定义一个输出文件流对象f，
 //它是与物理文件相对应的逻辑文件
 if(!f) {
  cerr<<fn<<' '<<"not found!"<<endl;exit(1);}
 for(int i=0;i<n;i++) 
 {//假定向每个记录的排序码域输入数据,其值由随机产生
  ElemType x;
  x.key=i;x.stn=random(299+i)%200;
  f.write((char *)&x,b);}
  f.close();}//关闭逻辑文件f
//采用选择排序法对文件A的记录进行排序
